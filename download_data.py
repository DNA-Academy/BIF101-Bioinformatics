...
"""
DNA Academy – BIF101 Veri İndirici (Enterprise Edition v2.1)
...
4) Manifest: Bilimsel izlenebilirlik (source_url, subset mode/target, final MB, sha256, timestamp).
"""
...
# Staphylococcus aureus (MRSA) - Eşleşmiş Numuneler (premium: doğrulanır)
ILLUMINA_ACCESSION = "ERR3336960"
NANOPORE_ACCESSION = "ERR3336961"

TARGET_MB_LONG = 200   # Nanopore hedef boyut (MB) – çıktı sıkıştırılmış boyut
TARGET_MB_SHORT = 50   # Illumina R1 hedef boyut (MB) – çıktı sıkıştırılmış boyut

# Premium kural: sample mismatch varsa dur (isterseniz True yapıp uyarı ile devam ettirebilirsiniz)
ALLOW_SAMPLE_MISMATCH = False
...
def detect_r1_r2(urls: List[str]) -> Tuple[List[str], List[str]]:
    ...
    for u in urls:
        ul = u.lower()
        if "_1.fastq" in ul or "_r1" in ul:
            r1.append(u)
        elif "_2.fastq" in ul or "_r2" in ul:
            r2.append(u)
    if not r1 and len(urls) >= 1:
        r1 = [urls[0]]
...
def stream_subsample(
    ...
    *,
    ...
    """
    mode="MB"    -> target MB (sıkıştırılmış çıktı boyutuna göre)
    mode="READS" -> target read sayısı
    Çıktı her zaman valid .fastq.gz üretilir.
    """
    ...
    try:
        ...
        # ENA fastq_ftp genellikle .fastq.gz döner. Yine de uzantıya göre güvenli davranalım.
...
def get_consortium_data():
    ...
    print("🚀 BIF101 Enterprise Veri İndirici (v2.1)\n")
    ...
    if not ill or not ont:
        print("❌ Metadata alınamadı. Çıkılıyor.")
        return
    ...
    if ill.sample_accession and ont.sample_accession and (ill.sample_accession != ont.sample_accession):
        msg = f"Numuneler eşleşmiyor! Illumina={ill.sample_accession} vs ONT={ont.sample_accession}"
        if not ALLOW_SAMPLE_MISMATCH:
            print(f"❌ KRİTİK: {msg}")
            print("   Premium kural: aynı izolat kıyası için sample_accession eşleşmeli.")
            return
        else:
            print(f"⚠️ UYARI: {msg} (ALLOW_SAMPLE_MISMATCH=True olduğu için devam ediliyor)")
    else:
        print(f"✅ Numune Eşleşmesi Doğrulandı: {ill.sample_accession or '(boş/gelmedi)'}")
    ...
    if not (r1_urls and r2_urls):
        print("❌ Illumina R1/R2 URL ayrıştırılamadı:", ill.fastq_urls)
        return

    ill_r1_name = f"{ill.run_accession}_ILLUMINA_R1_{TARGET_MB_SHORT}MB.fastq.gz"
    ill_r2_name = f"{ill.run_accession}_ILLUMINA_R2_matched.fastq.gz"
    ...
    if reads_r2 <= 0:
        print("❌ Illumina R2 indirilemedi.")
        return

    manifest_append({
        "filename": ill_r1_name,
        "role": "SHORT_R1",
        "run_accession": ill.run_accession,
        "sample_accession": ill.sample_accession,
        "study_accession": ill.study_accession,
        "platform": ill.instrument_platform,
        "instrument_model": ill.instrument_model,
        "library_layout": ill.library_layout,
        "organism": ill.scientific_name,
        "strain": ill.strain,
        "source_url": r1_urls[0],
        "subset_mode": "target_mb",
        "target": str(TARGET_MB_SHORT),
        "reads": str(reads_r1),
        "final_mb": f"{mb_r1:.2f}",
        "sha256": calculate_sha256(ill_r1_path),
        "created_utc": created_utc,
    })

    manifest_append({
        "filename": ill_r2_name,
        "role": "SHORT_R2",
        "run_accession": ill.run_accession,
        "sample_accession": ill.sample_accession,
        "study_accession": ill.study_accession,
        ...
        "organism": ill.scientific_name,
        "strain": ill.strain,
        "source_url": r2_urls[0],
        "subset_mode": "matched_reads",
        "target": str(reads_r1),
        "reads": str(reads_r2),
        "final_mb": f"{mb_r2:.2f}",
        "sha256": calculate_sha256(ill_r2_path),
        "created_utc": created_utc,
    })
    ...
    # 4) NANOPORE
    if not ont.fastq_urls:
        print("❌ Nanopore URL bulunamadı.")
        return

    ont_tag = "ONT" if ont.instrument_platform == "OXFORD_NANOPORE" else "LONGREAD"
    ont_name = f"{ont.run_accession}_{ont_tag}_{TARGET_MB_LONG}MB.fastq.gz"
    ...
    if reads_ont <= 0:
        print("❌ Nanopore indirilemedi.")
        return

    manifest_append({
        "filename": ont_name,
        "role": "LONGREAD",
        "run_accession": ont.run_accession,
        "sample_accession": ont.sample_accession,
        "study_accession": ont.study_accession,
        "platform": ont.instrument_platform,
        "instrument_model": ont.instrument_model,
        "library_layout": ont.library_layout,
        "organism": ont.scientific_name,
        "strain": ont.strain,
        "source_url": ont.fastq_urls[0],
        "subset_mode": "target_mb",
        "target": str(TARGET_MB_LONG),
        "reads": str(reads_ont),
        "final_mb": f"{mb_ont:.2f}",
        "sha256": calculate_sha256(ont_path),
        "created_utc": created_utc,
    })
